var books = {
  1: { title: "The Great Gatsby", author: "F. Scott Fitzgerald", genre: "Classic Fiction", price: 14.99, img: "https://covers.openlibrary.org/b/id/8231856-L.jpg", desc: "Set in the Jazz Age on Long Island, this novel depicts narrator Nick Carraway's interactions with the mysterious millionaire Jay Gatsby and his obsession with the beautiful Daisy Buchanan. A timeless story of wealth, love, and the American Dream." },
  2: { title: "To Kill a Mockingbird", author: "Harper Lee", genre: "Classic / Drama", price: 12.99, img: "https://covers.openlibrary.org/b/id/8091016-L.jpg", desc: "A gripping tale of racial injustice and childhood innocence set in the American South during the 1930s. Told through the eyes of young Scout Finch, whose father Atticus defends a Black man falsely accused of a crime." },
  3: { title: "1984", author: "George Orwell", genre: "Dystopian Fiction", price: 11.49, img: "https://covers.openlibrary.org/b/id/8235110-L.jpg", desc: "A chilling dystopia where Big Brother watches your every move. Winston Smith works for the Party rewriting history, but secretly dreams of rebellion. One of the most influential novels of the 20th century." },
  4: { title: "Pride and Prejudice", author: "Jane Austen", genre: "Romance / Classic", price: 9.99, img: "https://covers.openlibrary.org/b/id/8228691-L.jpg", desc: "The story follows Elizabeth Bennet as she deals with issues of manners, upbringing, morality, education, and marriage in the society of the landed gentry of early 19th-century England." },
  5: { title: "The Silent Patient", author: "Alex Michaelides", genre: "Psychological Thriller", price: 16.99, img: "https://covers.openlibrary.org/b/id/8281993-L.jpg", desc: "Alicia Berenson shoots her husband five times and then never speaks another word. Theo Faber, a criminal psychotherapist, is obsessed with uncovering her motive. A shocking twist awaits at the end." },
  6: { title: "Where The Crawdads Sing", author: "Delia Owens", genre: "Mystery / Drama", price: 13.49, img: "https://covers.openlibrary.org/b/id/8250995-L.jpg", desc: "Abandoned by her family, Kya Clark raises herself in the marshes of North Carolina. When a local man is found dead, she becomes the prime suspect. A beautiful story of nature, loneliness, and survival." },
  7: { title: "The Guest List", author: "Lucy Foley", genre: "Mystery / Thriller", price: 15.99, img: "https://covers.openlibrary.org/b/id/8226193-L.jpg", desc: "On a remote island off the Irish coast, a wedding celebration turns deadly. Everyone has a secret. Everyone has a motive. A gripping whodunit told from multiple perspectives." },
  8: { title: "The Midnight Library", author: "Matt Haig", genre: "Fantasy / Fiction", price: 12.99, img: "https://covers.openlibrary.org/b/id/8250634-L.jpg", desc: "Between life and death there is a library. Nora Seed finds herself there and discovers books that let her live out all the lives she could have lived. A moving story about regret, hope, and second chances." },
  9: { title: "Project Hail Mary", author: "Andy Weir", genre: "Science Fiction", price: 17.99, img: "https://covers.openlibrary.org/b/id/8255099-L.jpg", desc: "Ryland Grace wakes up alone on a spaceship with no memory of how he got there. He soon realizes he is on a solo mission to save Earth from an extinction-level threat. A thrilling and heartwarming sci-fi adventure." }
};
...
var DELIVERY_OPTIONS = [
  ...
  { id: "free", label: "Free Delivery (7-10 days)", price: 0.00 }
];
...
function getSelectedDelivery() {
  ...
  if (!sel) return 3.99;
  var opt = DELIVERY_OPTIONS.find(function (o) { return o.id === sel.value; });
...
function updateCartTotals() {
  ...
  document.getElementById("cartDeliveryCharge").textContent = delivery === 0 ? "Free" : "$" + delivery.toFixed(2);
...
function addToCart(id) {
  ...
  else { cart[id] = { id: id, title: book.title, price: book.price, img: book.img, qty: 1 }; }
  updateCartCount();
  renderCart();
...
function buildDeliveryOptions() {
  ...
  DELIVERY_OPTIONS.forEach(function (opt, i) {
    html += '<label class="cart-delivery-option">' +
      '<input type="radio" name="cartDelivery" value="' + opt.id + '"' + (i === 0 ? " checked" : "") + ' />' +
      '<span class="cart-delivery-label">' +
        '<span>' + opt.label + '</span>' +
        '<span class="cart-delivery-price">' + (opt.price === 0 ? "Free" : "$" + opt.price.toFixed(2)) + '</span>' +
      '</span>' +
    '</label>';
  });
  container.innerHTML = html;
...
function renderCheckoutSummary() {
  ...
  items.forEach(function (item) {
    html += '<div class="co-item">' +
      '<img src="' + item.img + '" alt="' + item.title + '" />' +
      '<div class="co-item-info"><span>' + item.title + '</span><span>x' + item.qty + '</span></div>' +
  ...
  document.getElementById("coDelivery").textContent = deliveryPrice === 0 ? "Free" : "$" + deliveryPrice.toFixed(2);
...
async function placeOrder() {
  ...
  try { await API.placeOrder(items, delivery, shipping); } catch (e) {}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("modalBuyBtn").addEventListener("click", function () {
    var id = parseInt(this.getAttribute("data-id"));
    if (id && books[id]) { addToCart(id); closeModal(); openCart(); }
  });
  ...
  document.getElementById("continueShopping").addEventListener("click", closeOrderSuccess);
  ...
  document.getElementById("membershipModalClose").addEventListener("click", function () {
    document.getElementById("membershipModal").classList.remove("active");
    document.body.style.overflow = "";
  });
  ...
  document.addEventListener("click", function (e) {
    if (e.target.classList.contains("btn-cart") && !e.target.closest(".modal-box")) {
      e.stopPropagation();
      var id = parseInt(e.target.getAttribute("data-id"));
      if (id) addToCart(id);
      return;
    }
    if (e.target.classList.contains("btn-buy") && !e.target.closest(".modal-box")) {
      e.stopPropagation();
      var id = parseInt(e.target.getAttribute("data-id"));
      if (id) { addToCart(id); openCart(); }
      return;
    }
  ...
  window.addEventListener("scroll", function () {
    document.getElementById("navbar").classList.toggle("sticky", window.scrollY > 66);
  });
  ...
  new Swiper(".book-card-slider", {
    ...
    loop: true,
    pagination: { el: ".book-card-slider .swiper-pagination", clickable: true },
    breakpoints: {
  ...
  new Swiper(".new-arrivals-slider", {
    ...
    spaceBetween: 20,
    loop: true,
    pagination: { el: ".new-arrivals-slider .swiper-pagination", clickable: true },
    breakpoints: {
      0:   { slidesPerView: 2, spaceBetween: 12 },
      480: { slidesPerView: 3, spaceBetween: 16 },
  ...
  new Swiper(".latest-additions-slider", {
    ...
    spaceBetween: 20,
    loop: true,
    pagination: { el: ".latest-additions-slider .swiper-pagination", clickable: true },
    breakpoints: {
      0:   { slidesPerView: 2, spaceBetween: 12 },
