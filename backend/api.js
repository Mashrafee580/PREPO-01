...
const API = {
  ...
  async register(name, email, password) {
    ...
    if (r.success) {
      localStorage.setItem("pt_token", r.data.token);
      localStorage.setItem("pt_user", JSON.stringify(r.data.user));
    }
    return r;
  },
  ...
  deleteListing(id) {
    return this.del("/marketplace/" + id);
  },
  contactSeller(id, data) {
    return this.post("/marketplace/" + id + "/contact", data);
  },

  // ── Membership ──
  subscribe(plan) {
    return this.post("/membership", { plan });
  },
  getMembership(userId) {
    return this.get("/membership/" + userId);
  },
};
